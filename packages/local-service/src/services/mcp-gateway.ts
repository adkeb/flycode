...
import type { McpGateway, ServiceContext } from "../types.js";
...
export const SUPPORTED_SITES: KnownSiteId[] = ["qwen", "deepseek", "gemini"];
...
export function createMcpGateway(context: ServiceContext): McpGateway {
  return {
    dispatch(site, envelope, traceId) {
      return dispatchMcp(site, envelope, traceId, context);
    },
    classifyStatus
  };
}
...
async function executeToolCall(input: {
  ...
  let args = input.args;
  const pendingPayload = input.confirmedPayload;

  if (pendingPayload?.kind === "tool-call" && pendingPayload.name === toolName) {
...
export function classifyStatus(response: McpResponseEnvelope): "success" | "failed" | "pending" {
  if (response.error) {
    return "failed";
  }
...
export function toMcpError(error: AppError): McpError {
  return {
...
export function assertKnownSite(raw: string): KnownSiteId {
